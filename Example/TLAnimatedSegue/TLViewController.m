...
//  TLAnimatedSegue
//
//  Created by BrunoBerisso on 02/11/2015.
...
/*
 ...
 This is an example implementation the TLAnimatedSegueDelegate. This example has two instance of this view controller whit a ciclic navigation declared on the storyboard (the first controller present the second that present the first again). So the animation is the same on both segues. This same effect can be achieved moving the animation logic to an "animator" object that perform the animation outside the controller. This idea can be extended so you can swap the animator at run time and perform different animations for the same navigation.
 
 For more details of how the TLAnimatedSegue work see the README.md
 
 */



@interface TLViewController () <TLAnimatedSegueDelegate>
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    //Here we set the delegate of the segue how is the responsable to perform the animation. This can be any object that conforms to the TLAnimatedSegueDelegate protocol.
...
//This is the core method of the TLAnimatedSegueDelegate. Here you receive the source and destination objects that perform the segue and a callback that you need to perform when your animation end.
...
//This method is part of the TLPresentSegueDelegate protocol that is a super protocol of TLAnimatedSegueDelgate. It allow to control how a controller will be presented. Here we are telling the segue that dismiss the visible controller _before_ present the new one. This avoid having a constantly growing list of controllers, this can produce a warning on the console about "Attempt to present <ViewController: 0xXXXXXXX> on <ViewController: 0xYYYYYY> whose view is not in the window hierarchy!".
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    ...
    //Perform the callback to let know the TLAnimatedSegue that we are done with the animation and procede with the presentation of the controller.
